...
class CreateBibleFilesetsTable extends Migration
{
    ...
    public function up()
    {
        ...
        if (!Schema::connection('dbp')->hasTable('bible_fileset_types')) {
            Schema::connection('dbp')->create('bible_fileset_types', function (Blueprint $table) {
                $table->tinyIncrements('id');
                $table->string('set_type_code', 18)->unique();
                $table->string('name')->unique();
                $table->timestamps();
            });
        }
        ...
        if (!Schema::connection('dbp')->hasTable('bible_filesets')) {
            Schema::connection('dbp')->create('bible_filesets', function (Blueprint $table) {
                ...
                $table->string('set_type_code', 18)->index();
    ...
    public function down()
    {
        ...
        Schema::connection('dbp')->dropIfExists('bible_fileset_copyrights');
        Schema::connection('dbp')->dropIfExists('bible_fileset_copyright_roles');
        Schema::connection('dbp')->dropIfExists('bible_fileset_copyright_organizations');
    }
}
